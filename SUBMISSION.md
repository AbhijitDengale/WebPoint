...
### What Waypoint does
Waypoint is a trip itinerary planner where the agent works *inside the same page*. It registers seven WebMCP tools (`get_itinerary`, `add_activities`, `update_activity`, `remove_activity`, `reorder_day`, `find_conflicts`, `get_budget_summary`), so an agent in ChatGPT's in-app browser (or a WebMCP-enabled browser) can read and modify the exact board you're looking at. Every agent action lands on your screen instantly, and every action from either side lands in one shared activity feed.
